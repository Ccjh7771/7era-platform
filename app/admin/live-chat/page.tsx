...
type AdminLiveChatPageProps = { searchParams: Promise<{ member?: string | string[] }> };

export default async function AdminLiveChatPage({ searchParams }: AdminLiveChatPageProps) {
  const admin = await requireAdmin();
  const params = await searchParams;
  const requestedValue = Array.isArray(params.member) ? params.member[0] : params.member;
  const requestedMemberId = requestedValue && /^[0-9a-f-]{36}$/i.test(requestedValue) ? requestedValue : null;
  const client = createAdminClient();
  const [conversationsResult, staffResult, requestedMemberResult] = await Promise.all([
    ...
    requestedMemberId
      ? client.from("member_profiles").select("id, full_name, phone").eq("id", requestedMemberId).maybeSingle()
  ...
  return (
    <section>
      ...
      <AdminLiveChat adminId={admin.id} canReply={admin.role !== "viewer"} initialConversations={conversations as never[]} initialMessages={(messagesResult.data ?? []) as never[]} staff={staff} requestedMember={requestedMember} />
    </section>
  );
}
