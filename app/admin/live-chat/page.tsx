...
export default async function AdminLiveChatPage() {
  ...
  const [conversationsResult, staffResult] = await Promise.all([
    client.from("chat_conversations").select("id, member_id, subject, status, assigned_admin_id, last_message_at, member_profiles(full_name, phone)").order("last_message_at", { ascending: false }).limit(250),
  ...
  const conversations = (conversationsResult.data ?? []).map((conversation) => {
    ...
    return { id: conversation.id, member_id: conversation.member_id, subject: conversation.subject, status: conversation.status, assigned_admin_id: conversation.assigned_admin_id, last_message_at: conversation.last_message_at, memberName: member?.full_name ?? "Member", memberPhone: member?.phone ? displayMalaysianPhone(member.phone) : "" };
  });
