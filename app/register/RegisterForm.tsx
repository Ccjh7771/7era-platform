...
export function RegisterForm() {
  ...
  if (state.status === "success") {
    return (
      <div className="rounded-[28px] border border-emerald-400/25 bg-emerald-400/10 p-6 sm:p-8">
        ...
        <h2 className="mt-3 text-2xl font-black text-white">Registration successful</h2>
        ...
        <p className="mt-4 text-xs leading-5 text-amber-200">Use the password you created during registration to sign in.</p>
  ...
  return (
    <form action={formAction} className="rounded-[28px] border border-white/10 bg-white/[0.04] p-6 shadow-2xl backdrop-blur-xl sm:p-8">
      ...
      <input id="member-password" name="password" type="password" required minLength={6} maxLength={72} autoComplete="new-password" placeholder="At least 6 characters" className="mt-3 h-14 w-full rounded-2xl border border-white/10 bg-black/50 px-5 text-white outline-none focus:border-yellow-400/50" />
      <label htmlFor="confirm-member-password" className="mt-6 block text-sm font-semibold text-zinc-200">Confirm password</label>
      <input id="confirm-member-password" name="confirmPassword" type="password" required minLength={6} maxLength={72} autoComplete="new-password" placeholder="Enter the same password again" className="mt-3 h-14 w-full rounded-2xl border border-white/10 bg-black/50 px-5 text-white outline-none focus:border-yellow-400/50" />
      <p className="mt-2 text-xs leading-5 text-zinc-500">Use at least 6 letters, numbers or a simple combination.</p>
